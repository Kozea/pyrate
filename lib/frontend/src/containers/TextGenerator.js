...
class TextGenerator extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    let selectedAlgo = (nextProps.algorithmes[0] !== undefined ) ? nextProps.algorithmes[0].label : "";
    let selectedCategory = (nextProps.categories[0] !== undefined) ? nextProps.categories[0].label : "";
  ...
  runGenText(event, type) {
    event.preventDefault();
    let catId = parseInt(this.state.selectedCategory);
    switch(type) {
        case 'generate':
      ...
      <div>
        <form onSubmit={(event) => this.formPreventDefault(event)} >
          ...
          <select name="algo"
           ...
           <button onClick={(event) => this.runGenText(event, 'train')}>Train</button>
           <button onClick={(event) => this.runGenText(event, 'generate')}>Generate</button>
