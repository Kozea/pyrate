...
export function updateCategory(catLabel, categoryId) {
  return function(dispatch) {
    return pyrateApi
      .addCategory(catLabel, categoryId)
      .then(result => {
        dispatch(updateCategorySuccess(catLabel, result.message))
      })
      .catch(error => {
        throw error
      })
...
export const updateCategorySuccess = (catLabel, message) => ({
